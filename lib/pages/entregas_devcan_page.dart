...
class _EntregasDevCanPageState extends State<EntregasDevCanPage> {
  ...
  Future<void> _sincronizarFirmasPendientes() async {
    ...
    final key = 'firmas_pendientes_devcan';
    final data = prefs.getString(key);
    if (data != null) {
      try {
        final List<dynamic> pendientes = jsonDecode(data);
        if (pendientes.isNotEmpty) {
          // Subir pendientes a la subcolección 'firmas' en Firestore
  ...
  Future<void> _cargarDatos({bool forzarFirestore = false}) async {
    ...
    if (forzarFirestore) {
      ...
      // Si el cache tiene distinto número de items que Firestore, actualizar cache
      try {
        final cachedItems =
        ...
        if (fsItems.length != cachedItems.length) {
          try {
            print(
                'DEBUG: entregas/devcan cache_size=${cachedItems.length} firestore_size=${fsItems.length} -> updating cache');
          } catch (_) {}
          entregasRaw = {'items': fsItems};
          await guardarDatosFirestoreYCache('entregas', 'devcan', entregasRaw);
        }
      } catch (_) {}
  ...
  Future<void> _firmarSeleccionados(BuildContext context) async {
    ...
    final resultado = await showDialog<Map<String, dynamic>>(
      ...
      builder: (ctx) {
        final isMobile = MediaQuery.of(ctx).size.shortestSide <= 600;
        if (isMobile) {
          return Dialog(
    ...
    try {
      // Preparar objetos nuevos y sus docIds
      preparados = [];
      ...
      // Ejecutar una única transacción: escribir todas las firmas en historial
      // y eliminar solo los ids seleccionados del documento 'entregas/devcan'
      ...
      // Guardar los preparados (o lo que ya se haya añadido a nuevasFirmadas)
